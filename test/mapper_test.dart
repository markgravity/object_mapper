...
void expectInfo(Map<String,dynamic> json, info) {
	...
	expect(info.time, DateTime.fromMillisecondsSinceEpoch(json["time"].toInt()*1000));
...
void expectJson(Map<String,dynamic> json, info) {
	...
	expect(DateTime.fromMillisecondsSinceEpoch(json["time"].toInt()*1000), info.time);
...
void main() {
	...
	group("toObject", (){
		...
		test("#2 Correct model class", () {
			...
			expect(info.times.first.millisecondsSinceEpoch, equals(json["times"][0] * 1000));
	...
	test("toJson", () {
		...
		info.times = [DateTime.fromMillisecondsSinceEpoch(324*1000), DateTime.fromMillisecondsSinceEpoch(325*1000)];
		info.meta = { "empty": "no" };
